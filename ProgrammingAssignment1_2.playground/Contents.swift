...
for student in studentsList {
    var info = student.components(separatedBy: " ")
    if info[0] == ""{
        info.remove(at: 0)
    }
    let name: String = info[0] + " " + info[1]
    let keyExists = studentsGroups[info[info.count - 1]] != nil
    if keyExists{
        studentsGroups[info[info.count - 1]]! += [name]
    }else{
        studentsGroups[info[info.count - 1]] = [name]
    }
}
...
for (key, value) in studentsGroups {
    var groupList: [String: [Int]] = [:]
    
    for student in value {
        groupList[student] = [Int]()
        for mark in points {
            groupList[student]!.append(randomValue(maxValue: mark))
        }
    }
    studentPoints[key] = groupList
}
// Ваш код закінчується тут
...
for (key, value) in studentPoints{
    ...
    for (key_1, value_1) in value{
        var average = 0
        for point in value_1{
            average += point
        }
        groupList[key_1] = average
    }
    sumPoints[key] = groupList
}
// Ваш код закінчується тут
...
for (key, value) in sumPoints{
    var avarege: Float = 0
    for (_, point) in value{
        avarege += Float(point) / Float(value.count)
    }
    
    groupAvg[key] = avarege
}
// Ваш код закінчується тут
